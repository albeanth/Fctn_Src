...
# #=========================#
# fun = lambda x: sin(x)
# fun_p = lambda x: cos(x)
# for i,ne in enumerate(NumOfElem):
#     testGrid = SetUpGrid.CFEMGrid1D([0.0,pi],[ne],1)
#     FESoln = BVP.CFEM_Func_1D(testGrid,fun)
#     l2Err, h1Err = BVP.Error(testGrid,FESoln,fun,fun_p)
#     L2Error[i] = l2Err
#     H1Error[i] = h1Err
#     h[i] = testGrid.hmax
...
#     MaxErr = BVP.Linfty(testGrid,FESoln,T)
#     integL2 = FunT.L2_Norm(testGrid,FESoln)
#     print('{0:.8e}, {1:.8e}'.format(l2Err,MaxErr))
#     L2Error[i] = l2Err
#     H1Error[i] = h1Err
#     h[i] = testGrid.hmax
...
#    DFEM BVP TEST   #
#====================#
# DG_L2Error = np.ones(len(NumOfElem))
# DG_H1Error = np.ones(len(NumOfElem))
# T = lambda x: x**2 #sin(x)
# Tp = lambda x: 2*x #cos(x)
# source = lambda x: -4*x #cond(x)*sin(x) - cos(x)
# cond = lambda x: x
...
FunT.Spatial_Convergence(L2Error, H1Error, h, True)
# pltCnt = FunT.ConvergencePlots(h,L2Error,H1Error,pltCnt)
