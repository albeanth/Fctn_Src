      ...
      for (int Bel=0; Bel<nelsB; Bel++){
        ...
        for (int l2=0; l2<qpsB.nw; l2++){
          ...
          for(int Cel=0; Cel<nelsC; Cel++){
            ...
            cR = xnodC[nodC[Cel][orderC[Cel]-1]];
            if (flag != 'E'){
              dc = (log10(xnodC[nodC[Cel][orderC[Cel]-1]]) - log10(xnodC[nodC[Cel][0]]) )/2.0;
            }
            else{
              dc = (cR-cL)/2.0;
            }
